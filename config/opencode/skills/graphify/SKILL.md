...
Before executing any command, check whether a `<system-reminder>` block containing `Plan mode ACTIVE` is present in your current context. If it is, you are in **read-only mode** and MUST NOT run any pipeline step that writes files or mutates persistent state.

**Forbidden in read-only mode (all steps that write to disk):**
- The full `/graphify <path>` build pipeline (Steps 1–9)
- `/graphify --update` (re-extraction and graph merge)
- `/graphify --cluster-only` (rewrites `graph.json` and `GRAPH_REPORT.md`)
- `/graphify add <url>` (fetches and saves to `./raw`, triggers update)
- `/graphify --watch` (background watcher that auto-writes on change)
- Any step that writes `graph.json`, `GRAPH_REPORT.md`, `graphify-out/`, obsidian vault, HTML, SVG, GraphML, Cypher, or cache files
- Temp pipeline files (`.graphify_chunk_*.json`, `.graphify_semantic.json`, etc.) — do not write even these in read-only mode

**Allowed in read-only mode (query against an existing graph):**
- `/graphify query "<question>"` — reads `graphify-out/graph.json`, outputs to stdout only; skip the `save-result` write-back step
- `/graphify path "A" "B"` — reads graph, outputs to stdout only; skip the `save-result` write-back step
- `/graphify explain "<node>"` — reads graph, outputs to stdout only; skip the `save-result` write-back step
- `/graphify --help` / `/graphify -h`

If the user requests a pipeline or write operation while in read-only mode, respond:
> "I am currently in plan/read-only mode. I can describe what this operation would do, but I will not build or modify the graph until plan mode ends."
